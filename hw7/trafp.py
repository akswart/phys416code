...
ip = np.zeros(number_of_cars,dtype=int)
ip[0:number_of_cars-1] = np.arange(1,number_of_cars)
ip[-1] = 1;
temp=xcar[ip]-xcar[0:]
density = car_len/np.mod((xcar[ip]-xcar),xmax);
...
# now if the car leaves the rhs make it enter from the left
    xcar = np.mod(xcar+xmax/2.,xmax)-xmax/2;
