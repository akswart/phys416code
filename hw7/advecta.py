...
method = int(input('Choose a numerical method: (1)FTCS, (2)Lax, (3)Lax-Wendroff, (4)Upwind, (5)Downwind: '))
...
animate = int(input('Enter a 1 to make an animaton: '))
...
for iStep in range(1,nStep+1):  ## MAIN LOOP ##
  ...
  #  Lax or Lax-Wendroff method.
    ...
    elif(method==5):                  ### Downwind method ###
        a[0:N] = a[0:N] + 2.*coeff*(a[ip]-a[0:N]) 
...
methodlabel={1:'FTCS',2:'Lax',3:'Lax-Wendroff',4:'Upwind',5:'Downwind'}
...
plt.xlabel('x')
plt.ylabel('a(x,t)')
plt.title(methodlabel[method]+r' $\tau=$'+str(tau))
plt.grid(True)
plt.show()
