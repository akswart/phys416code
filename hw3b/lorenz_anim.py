...
for i in range(0,imax):
    ay.plot3D(xplot[0:i],yplot[0:i],zplot[0:i],'b-')
    ay.set_xlabel('x')
    ay.set_ylabel('y')
    ay.set_zlabel('z')
    plt.title('Lorenz phase space, r= '+str(r)+\
           ' it='+str(i)+'/'+str(imax));
    # aa.view([30,20]);
    ay.grid(True);
    ay.set_xlim3d(np.min(xplot),np.max(xplot))
    ay.set_ylim3d(np.min(yplot),np.max(yplot))
    ay.set_zlim3d(np.min(zplot),np.max(zplot))
    ay.set_aspect('auto')
    plt.draw()
    plt.show()
    plt.pause(0.01)
    if(i<imax-1):ay.clear() # fixes title problem
plt.ioff()
